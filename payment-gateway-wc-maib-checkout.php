    ...
    class WC_Gateway_MAIB_Checkout extends WC_Payment_Gateway
    {
        ...
        public function validate_required_field($key, $value)
        {
            if (empty($value)) {
                ...
                WC_Admin_Settings::add_error(esc_html(sprintf(__('%1$s field must be set.', 'payment-gateway-wc-maib-checkout'), $this->get_settings_field_label($key))));
            }
        ...
        protected function confirm_payment(\WC_Order $order, array $payment_data, array $payment_receipt_data)
        {
            ...
            if ($order_price !== $payment_data_price) {
                ...
                $message = sprintf(__('Order amount mismatch: Payment: %1$s, Order: %2$s.', 'payment-gateway-wc-maib-checkout'), $payment_data_price, $order_price);
        ...
        protected function get_callback_url()
        {
            ...
            return apply_filters('maib_checkout_callback_url', $callback_url);
        }
