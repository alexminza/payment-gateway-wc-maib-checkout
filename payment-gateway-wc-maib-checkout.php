...
if (!defined('WC_MAIB_CHECKOUT_PLUGIN_FILE')) {
    define('WC_MAIB_CHECKOUT_PLUGIN_FILE', __FILE__);
}
...
function maib_checkout_plugins_loaded_init()
{
    ...
    require_once plugin_dir_path(WC_MAIB_CHECKOUT_PLUGIN_FILE) . 'includes/class-wc-gateway-maib-checkout.php';

    //region Init payment gateway
    add_filter('woocommerce_payment_gateways', array(WC_Gateway_MAIB_Checkout::class, 'add_gateway'));

    if (is_admin()) {
        add_filter('plugin_action_links_' . plugin_basename(WC_MAIB_CHECKOUT_PLUGIN_FILE), array(WC_Gateway_MAIB_Checkout::class, 'plugin_action_links'));
...
add_action(
    ...
    function () {
        if (class_exists(\Automattic\WooCommerce\Utilities\FeaturesUtil::class)) {
            ...
            \Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility('custom_order_tables', WC_MAIB_CHECKOUT_PLUGIN_FILE, true);

            // WooCommerce Cart Checkout Blocks compatibility
            // https://github.com/woocommerce/woocommerce/pull/36426
            \Automattic\WooCommerce\Utilities\FeaturesUtil::declare_compatibility('cart_checkout_blocks', WC_MAIB_CHECKOUT_PLUGIN_FILE, true);
        }
    }
);
//endregion
...
add_action(
    ...
    function () {
        if (class_exists(\Automattic\WooCommerce\Blocks\Payments\Integrations\AbstractPaymentMethodType::class)) {
            require_once plugin_dir_path(WC_MAIB_CHECKOUT_PLUGIN_FILE) . 'includes/class-wc-gateway-maib-checkout-wbc.php';
